...
fn encrypting_file() {
    ...
    use ziperu::unstable::write::FileOptionsExt;
    let mut buf = vec![0; 2048];
    let mut archive = ziperu::write::ZipWriter::new(std::io::Cursor::new(&mut buf));
    archive
        .start_file(
            "name",
            ziperu::write::FileOptions::default().with_deprecated_encryption(b"password"),
        )
        .unwrap();
    archive.write_all(b"test").unwrap();
    archive.finish().unwrap();
    drop(archive);
    let mut archive = ziperu::ZipArchive::new(std::io::Cursor::new(&mut buf)).unwrap();
...
fn encrypted_file() {
    ...
    let mut archive = ziperu::ZipArchive::new(zip_file_bytes).unwrap();
...

    {
        ...
        match file {
            Err(ziperu::result::ZipError::UnsupportedArchive(
                ziperu::result::ZipError::PASSWORD_REQUIRED,
            )) => (),
            Err(_) => panic!(
...

    {
        ...
        match file {
            Ok(Err(ziperu::result::InvalidPassword)) => (),
            Err(_) => panic!(
...
// See https://github.com/zip-rs/zip-old/issues/280
#[test]
fn issue_zip_old_280() {
    // buffer capacity needs to be between 13..=46 bytes with exception of 44 bytes for this file
    let zip_file_bytes = &mut Cursor::new(ZIP_CRYPTO_FILE);
    let buffered = BufReader::with_capacity(13, zip_file_bytes);
    let mut archive = ziperu::ZipArchive::new(buffered).unwrap();

    let mut file = archive.by_index_decrypt(0, b"test").unwrap().unwrap();
