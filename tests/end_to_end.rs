...
use ziperu::{CompressionMethod, SUPPORTED_COMPRESSION_METHODS};
...
fn copy() {
    for &method in SUPPORTED_COMPRESSION_METHODS {
        ...
        {
            let mut src_archive = ziperu::ZipArchive::new(src_file).unwrap();
            let mut zip = ziperu::ZipWriter::new(&mut tgt_file);
        ...
        let mut tgt_archive = ziperu::ZipArchive::new(tgt_file).unwrap();
...
fn append() {
    for &method in SUPPORTED_COMPRESSION_METHODS {
        ...
        {
            let mut zip = ziperu::ZipWriter::new_append(&mut file).unwrap();
            zip.start_file(
                COPY_ENTRY_NAME,
        ...
        let mut zip = ziperu::ZipArchive::new(&mut file).unwrap();
...
fn write_test_archive(
    ...
    let mut zip = ziperu::ZipWriter::new(file);
...
fn read_archive_file<R: Read + Seek>(
    archive: &mut ziperu::ZipArchive<R>,
    name: &str,
) -> ziperu::result::ZipResult<String> {
